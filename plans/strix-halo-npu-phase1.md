# Strix Halo deployment ΓÇö Phase 1 (NPU STT, iGPU TTS)
...
- **Phase 1 (this doc):** Whisper STT on the XDNA 2 NPU, Kokoro TTS
  ...
  AMD's stack ΓÇö we just wire Lemonade to it.
- **Phase 2 (future):** Kokoro TTS on the NPU, iGPU fully freed for
  LLM / rendering / other workloads. No CPU fallback, no iGPU hack ΓÇö
...
this doc also applies to Strix Point laptops ΓÇö just point the GPU
...
from AMD, install. Open Task Manager ΓåÆ Performance tab and confirm
you see **NPU0** alongside CPU and GPU.

Note: Ryzen AI 1.7 keeps the same driver package as 1.5 ΓÇö the WHQL
...
Same as the main README ΓÇö `pip install --index-url
...
`LEMONADE_WHISPERCPP_NPU_BIN` to its path ΓÇö see the runtime config
section below.
...
4. First transcription after that triggers Vitis AI's JIT compile of
   the encoder graph (~10-15 seconds the very first time ΓÇö the result
...
on NPU is the sweet spot ΓÇö live transcription with headroom for the
...
1. Device Manager ΓåÆ System devices ΓåÆ you should see "NPU Compute
...
If all three are fine, check the lemond log for the specific error ΓÇö
...
Would be surprising ΓÇö we explicitly fixed this (see the STFT-tile
