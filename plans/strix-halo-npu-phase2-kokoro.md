...
## DEFINITIVE FINDING (2026-04-23): STX NPU only supports CNN INT8

From AMD's official Model Compatibility Table in the Ryzen AI 1.7.1
release notes (https://ryzenai.docs.amd.com/en/latest/relnotes.html):
...
**Strix / Strix Halo / Krackan NPUs only support CNN INT8 models
through the generic VitisAI EP.** BF16 CNN, NLP BF16, and LLMs are
NOT supported on STX hardware with Ryzen AI SDK 1.7.1.

Kokoro-82M is a complex TTS model (ALBERT encoder + LSTM predictor +
iSTFTNet decoder) — it is NOT a simple CNN and does not fall into any
of the supported model categories for STX NPU.

### What we tried

1. FP32 static-shape model: compiler runs, produces .rai cache (94 MB),
   but XRT crashes with 0xC0000005 during HW context creation
2. FP16 static model: loads OK but all 2138 ops on CPU (fail_safe)
3. INT8 quantized model: same XRT crash as FP32
4. ORT-optimized model: compiler creates 32 NPU partitions (PT0-PT31),
   AIE2P code compiles successfully, but XRT crash at context load
5. ep_context_enable: fail_safe, 0 NPU ops

The Whisper NPU path works because amd/whisper.cpp has a CUSTOM
VitisAI integration (not the generic EP) that was specifically built
and validated for STX. It does not go through the same onnxruntime
VitisAI EP path we are using for Kokoro.

### Path forward

**Option A (wait):** AMD may add BF16/NLP support for STX in Ryzen AI
SDK 1.8 or 2.0. The compiler CAN produce STX code (we verified it
compiles), so this is a runtime enablement gap, not a fundamental
hardware limitation.

**Option B (custom integration):** Port Kokoro to use the same custom
VitisAI integration that amd/whisper.cpp uses — bypass the generic
onnxruntime EP entirely and interface directly with the FlexML
runtime and xrt_coreutil APIs. This is a deep engineering project.

**Option C (iGPU is fine):** Kokoro on the gfx1151 iGPU runs at 0.41s
warm with our HIP backend. This is fast enough for real-time TTS.
The NPU port would only be justified if the iGPU needs to be freed
for other work (LLM inference, rendering, etc.).
